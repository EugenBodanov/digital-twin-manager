...
class ColdS3BucketDeployer(Deployer):
  def log(self, message):
    print(f"Core: {message}")

  def plan(self):
    previous_bucket_name = deployment_state.last_applied_cold_s3_bucket_name()
    desired_bucket_name = globals.cold_s3_bucket_name()
    ...
    self.log(
      "Cold S3 Bucket will be redeployed: "
  ...
  def deploy(self, bucket_name=None):
    bucket_name = bucket_name or globals.cold_s3_bucket_name()
  ...
  def info(self):
    ...
    try:
      ...
      self.log(f"✅ Cold S3 Bucket exists: {util.link_to_s3_bucket(bucket_name)}")
    except ClientError as e:
      if int(e.response["Error"]["Code"]) == 404:
        self.log(f"❌ Cold S3 Bucket missing: {bucket_name}")
      else:
        raise
