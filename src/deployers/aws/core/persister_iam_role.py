...
class PersisterIamRoleDeployer(Deployer):
  ...
  def info(self):
    ...
    try:
      ...
      self.log(f"✅ Persister IAM Role exists: {util.link_to_iam_role(role_name)}")
    except ClientError as e:
      if e.response["Error"]["Code"] == "NoSuchEntity":
        self.log(f"❌ Persister IAM Role missing: {role_name}")
      else:
        raise
